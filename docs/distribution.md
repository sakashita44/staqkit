...
staqkit が目指す姿と、どう配布し、どう改善を伝播させるかを定める。
...
「解析リポジトリ側で加えた改善を取り込みたい」対象の大半は、本来は雛形ではなく staqkit パッケージに置くべきものである。振る舞いがパッケージにあればバージョン更新だけで伝播し、雛形同期の問題自体が生じない。したがって雛形は「パッケージに置けない、各プロジェクト固有の最小ボイラープレート」だけに痩せさせる。雛形が薄いほど伝播の悩みは小さい。

痩せた雛形の配布には Copier を用いる。Copier は生成時の回答とテンプレートのバージョンを記録し、`copier update` でテンプレート側の変更を既存プロジェクトにマージ適用できる。これにより「テンプレートを改善し、既存プロジェクトへ取り込む」という要件を満たす。

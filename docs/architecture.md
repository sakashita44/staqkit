...
要求 U2（多軸クエリ面）はフレームワークがドメインの識別軸構造を知ることを要求するが、ドメイン知識への依存は汎用性を損なう。この問題に対し、ドメイン依存を Project 層に局所化し、Core 層をドメイン非依存に保つ。識別軸の構造は DDL の PK/FK 制約から導出し、フレームワークがドメイン固有の語彙を予約語として持たない設計とする。

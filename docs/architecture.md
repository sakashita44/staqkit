...
探索もDAG内の操作として表現される。「探索」= 仮説的なノードと辺を作る操作、「確定」= 実データを生成してvalid状態にする操作。いずれも状態遷移として表現される。
...
- **ファイル形式・CLI**: 硬い契約。stage.yaml・table_schemas の DDL・ディレクトリ規約・CLI コマンドは壊さない。第三者が内部を読まずに再現・理解・拡張するとき実際に触れるのはこの面である（信頼・理解・独立の三観点の読み手はファイルと CLI を見る）
...
再現性の中核は Git と DVC へ委譲する（[requirements.md](requirements.md#委譲する関心とツール)）。DataStore の API 安定性はこれに寄与しない。`uv.lock` は各スナップショット時点の依存関係バージョン（staqkit 自体を含む）を固定し、API が将来変化した場合でも記録された staqkit バージョンでの再実行を可能にする。
...
パラメータの値は stage.yaml に書かず、DVC が追跡する外部 params ファイルに置く。stage.yaml の `params` は「ステージローカル名 → どのファイルのどのキーを引くか」の束縛だけを宣言し、値を持たない（[stage.md](components/stage.md#params外部ファイル参照)）。パラメータはステージに帰属せず、値の SSoT は params ファイルに一本化される。複数ステージが同じファイル・キーを参照すれば値を共有でき、共有のための専用構文も「定義元ステージ」の概念も持たない。run.py は宣言したローカル名だけを `stage.params` 経由で平らに読み、params ファイルの物理配置やネスト構造には依存しない。
...
| 設計原則         | 実現手段                                                                                             |
| ---------------- | ---------------------------------------------------------------------------------------------------- |
| 宣言の先行       | 実体に先立つ宣言（DDL によるスキーマ宣言、planned ノード、出力 IF 定義）、設定ファイル自身の契約検証 |
